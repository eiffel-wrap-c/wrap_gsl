...
feature -- Access
	...
	gsl_vector_complex_fread (stream: IOBUF_STRUCT_API; v: GSL_VECTOR_COMPLEX_STRUCT_API): INTEGER 
		do
			Result := c_gsl_vector_complex_fread (stream.item, v.item)
		end

	gsl_vector_complex_fwrite (stream: IOBUF_STRUCT_API; v: GSL_VECTOR_COMPLEX_STRUCT_API): INTEGER 
		do
			Result := c_gsl_vector_complex_fwrite (stream.item, v.item)
		end

	gsl_vector_complex_fscanf (stream: IOBUF_STRUCT_API; v: GSL_VECTOR_COMPLEX_STRUCT_API): INTEGER 
		do
			Result := c_gsl_vector_complex_fscanf (stream.item, v.item)
		end

	gsl_vector_complex_fprintf (stream: IOBUF_STRUCT_API; v: GSL_VECTOR_COMPLEX_STRUCT_API; format: STRING): INTEGER 
		local
			format_c_string: C_STRING
		do
			create format_c_string.make (format)
			Result := c_gsl_vector_complex_fprintf (stream.item, v.item, format_c_string.item)
		end
