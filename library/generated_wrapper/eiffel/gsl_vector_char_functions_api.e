...
feature -- Access
	...
	gsl_vector_char_fread (stream: IOBUF_STRUCT_API; v: GSL_VECTOR_CHAR_STRUCT_API): INTEGER 
		do
			Result := c_gsl_vector_char_fread (stream.item, v.item)
		end

	gsl_vector_char_fwrite (stream: IOBUF_STRUCT_API; v: GSL_VECTOR_CHAR_STRUCT_API): INTEGER 
		do
			Result := c_gsl_vector_char_fwrite (stream.item, v.item)
		end

	gsl_vector_char_fscanf (stream: IOBUF_STRUCT_API; v: GSL_VECTOR_CHAR_STRUCT_API): INTEGER 
		do
			Result := c_gsl_vector_char_fscanf (stream.item, v.item)
		end

	gsl_vector_char_fprintf (stream: IOBUF_STRUCT_API; v: GSL_VECTOR_CHAR_STRUCT_API; format: STRING): INTEGER 
		local
			format_c_string: C_STRING
		do
			create format_c_string.make (format)
			Result := c_gsl_vector_char_fprintf (stream.item, v.item, format_c_string.item)
		end
	...
	gsl_vector_char_scale (a: GSL_VECTOR_CHAR_STRUCT_API; x: REAL_64): INTEGER 
		do
...
feature -- Externals
	...
	c_gsl_vector_char_scale (a: POINTER; x: REAL_64): INTEGER
		external
			"C inline use <eif_gsl.h>"
		alias
			"[
				return gsl_vector_char_scale ((gsl_vector_char*)$a, (double const)$x);
			]"
		end
