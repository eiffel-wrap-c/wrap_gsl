...
feature -- Access
	...
	gsl_vector_fread (stream: FILE; v: GSL_VECTOR_STRUCT_API): INTEGER
		do
			Result := c_gsl_vector_fread (stream.file_pointer, v.item)
		ensure
			instsance_free: class
		end

	gsl_vector_fwrite (stream: FILE; v: GSL_VECTOR_STRUCT_API): INTEGER
		do
			Result := c_gsl_vector_fwrite (stream.file_pointer, v.item)
		ensure
			instsance_free: class
		end

	gsl_vector_fscanf (stream: FILE; v: GSL_VECTOR_STRUCT_API): INTEGER
		do
			Result := c_gsl_vector_fscanf (stream.file_pointer, v.item)
		ensure
			instsance_free: class
		end

	gsl_vector_fprintf (stream: FILE; v: GSL_VECTOR_STRUCT_API; format: STRING): INTEGER
		local
			format_c_string: C_STRING
		do
			create format_c_string.make (format)
			Result := c_gsl_vector_fprintf (stream.file_pointer, v.item, format_c_string.item)
		ensure
			instsance_free: class
		end
	...
	gsl_vector_axpby (alpha: REAL_64; x: GSL_VECTOR_STRUCT_API; beta: REAL_64; y: GSL_VECTOR_STRUCT_API): INTEGER
		do
			Result := c_gsl_vector_axpby (alpha, x.item, beta, y.item)
		ensure
			instsance_free: class
		end
...
feature -- Externals
	...
	c_gsl_vector_axpby (alpha: REAL_64; x: POINTER; beta: REAL_64; y: POINTER): INTEGER
		external
			"C inline use <eif_gsl.h>"
		alias
			"[
				return gsl_vector_axpby ((double const)$alpha, (gsl_vector const*)$x, (double const)$beta, (gsl_vector*)$y);
			]"
		end
