...
feature -- Access
	...
	gsl_vector_float_fread (stream: FILE; v: GSL_VECTOR_FLOAT_STRUCT_API): INTEGER
		do
			Result := c_gsl_vector_float_fread (stream.file_pointer, v.item)
		end

	gsl_vector_float_fwrite (stream: FILE; v: GSL_VECTOR_FLOAT_STRUCT_API): INTEGER
		do
			Result := c_gsl_vector_float_fwrite (stream.file_pointer, v.item)
		end

	gsl_vector_float_fscanf (stream: FILE; v: GSL_VECTOR_FLOAT_STRUCT_API): INTEGER
		do
			Result := c_gsl_vector_float_fscanf (stream.file_pointer, v.item)
		end

	gsl_vector_float_fprintf (stream: FILE; v: GSL_VECTOR_FLOAT_STRUCT_API; format: STRING): INTEGER
		local
			format_c_string: C_STRING
		do
			create format_c_string.make (format)
			Result := c_gsl_vector_float_fprintf (stream.file_pointer, v.item, format_c_string.item)
		end
	...
	gsl_vector_float_scale (a: GSL_VECTOR_FLOAT_STRUCT_API; x: REAL): INTEGER
		do
	...
	gsl_vector_float_axpby (alpha: REAL; x: GSL_VECTOR_FLOAT_STRUCT_API; beta: REAL; y: GSL_VECTOR_FLOAT_STRUCT_API): INTEGER
		do
			Result := c_gsl_vector_float_axpby (alpha, x.item, beta, y.item)
		end
...
feature -- Externals
	...
	c_gsl_vector_float_scale (a: POINTER; x: REAL): INTEGER
		external
			"C inline use <eif_gsl.h>"
		alias
			"[
				return gsl_vector_float_scale ((gsl_vector_float*)$a, (float const)$x);
			]"
		end
	...
	c_gsl_vector_float_axpby (alpha: REAL; x: POINTER; beta: REAL; y: POINTER): INTEGER
		external
			"C inline use <eif_gsl.h>"
		alias
			"[
				return gsl_vector_float_axpby ((float const)$alpha, (gsl_vector_float const*)$x, (float const)$beta, (gsl_vector_float*)$y);
			]"
		end
